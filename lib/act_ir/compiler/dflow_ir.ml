...
module Rui = struct
  ...
  let lower presynth ~new_chan =
    let new_ored_guard guards =
      ...
      | guards ->
          let v = new_chan 1 in
          let expr =
            List.map guards ~f:(fun v -> Ir.Expr.Var v)
            |> List.reduce_exn ~f:(fun a b -> Ir.Expr.BitOr (a, b))
          in
  ...
  let synth_opt_guarded_op_seq guarded_aliases ~new_chan ~dir =
    ...
    let ct_const = Ir.Expr.Const (CInt.of_int ct) in
      ...
      let dflows, gis =
        List.map guarded_aliases ~f:(fun (g_chan, _) ->
            ...
            | None ->
                ...
                ([ assign gi Ir.Expr.(Const CInt.one) ], gi))
        |> List.unzip
      in
    ...
    let dflow2 =
      ...
      in
      let sg_expr = Ir.Expr.(BitOr (Eq0 (Var g), Eq (Var n, ct_const))) in
    ...
    let dflow3 =
      let n1 = new_chan n_width in
      let sg = new_chan 1 in
      let sg_expr = Ir.Expr.(BitOr (Var g, Eq (Var n, ct_const))) in
      let b_expr = Ir.Expr.(Ne (Var n1, ct_const)) in
      [
  ...
  let synth_seq2 ~(rui1 : Alias_with_ctrl.t) ~(rui2 : Alias_with_ctrl.t)
    ...
    let dflow2 =
      let g = new_chan 1 in
      assign g (Ir.Expr.BitOr (Var s, Var v))
    ...
    let dflow3 =
      ...
      [
        ...
        [ assign s0' (Ir.Expr.BitXor (Var s0, Const CInt.one)) ];
  ...
  let synth_select_imm guards aliases ~new_chan ~dir =
    ...
    let dflow1 =
      ...
      let c_expr =
        Ir.Expr.Concat (List.map guards ~f:(fun e -> (Ir.Expr.Var e, 1)))
      in
      [
...
let dflow_of_stf proc =
  ...
  let of_e e = Ir.Expr.map_vars e ~f:(fun v -> of_v v) in
  let of_e' (e : Stf.Var.t Ir.Expr.t) =
    let bitwidth = Ir.Expr.bitwidth e ~bits_of_var:(fun v -> v.bitwidth) in
    ...
    List.map proc.oports ~f:(fun (interproc, chan) ->
        ...
        let alias, dflows =
          ...
          | None ->
              ...
              ( tmp,
                [ MultiAssign (FBlock.create1 tmp (Ir.Expr.Const CInt.zero)) ]
              )
        in
...
(* let cluster_fuse_chains { Proc.stmt = dflows; iports; oports } = let dflows =
   ...
   Ir.Expr.var_ids) |> List.filter_map ~f:(Map.find id_of_output) |>
   ...
   ~f:(fun (_, e) -> Ir.Expr.var_ids e) | Split (g, v, os) -> (v :: Guard.ids g)
   ...
   reads_of_id_o = List.concat_map multi_assign ~f:(fun (_, e) ->
   Ir.Expr.var_ids e) |> List.filter_map ~f:(Map.find cluster_of_output) |>
   Int.Set.of_list in let o_only_reader_of_i = Int.equal 1 (Map.find
   read_ct_of_cluster id_i |> Option.value ~default:0) && Set.mem reads_of_id_o
   id_i in let o_only_reads_of_i = List.equal Int.equal (Set.to_list
   reads_of_id_o) [ id_i ] in if o_only_reader_of_i || o_only_reads_of_i then
   Some (id_i, id_o) else None)) in let clusters = let ids = List.map
   multi_assigns ~f:fst |> Int.Set.of_list in connected_components ids fuses in
  ...
  let guards =
    ...
    |> Guard.Map.of_key_set ~f:(fun g ->
           ...
           | One_hot g ->
               ...
               let expr = Ir.Expr.Log2OneHot (Var g) in
           ...
           | Bits gs ->
               ...
               let gs = List.map gs ~f:(fun g -> (Ir.Expr.Var g, 1)) in
               let assign =
...
(* let const_vars = List.concat_map dflows ~f:(fun dflow -> match dflow with |
   MultiAssign l -> if List.concat_map l ~f:(fun (_, e) -> Ir.Expr.var_ids e) |>
   ...
   ~f:(fun (dst, e) -> let e = Ir.Expr.bind_vars e ~f:(fun v -> match Map.find
    ...
    List.concat_map couples ~f:(fun (split, merge) ->
        ...
        | Split (g, in_v, os), Merge (_, ins, out_v) -> (
            ...
            | Bits gs ->
                ...
                let parrellel_expr =
                  List.map parrellel ~f:(fun (g, _, _) -> Ir.Expr.Var g)
                  |> List.reduce_exn ~f:(fun a b -> Ir.Expr.BitOr (a, b))
                in
        ...
        | Split (g, i, os) -> (
            ...
            | Bits gs ->
                ...
                if List.is_empty non_sinks then []
                  ...
                  let g = new_chan 1 in
                  let expr =
                    List.map gs ~f:(fun g -> Ir.Expr.Var g)
                    |> List.reduce_exn ~f:(fun a b -> Ir.Expr.BitOr (a, b))
                  in
