...
type ('kind, 'a) t = { dtype : 'a Ir_dtype.t; m : Act_ir.Ir.Mem.t }
[@@deriving sexp_of]
...
let create dtype init creation_code_pos kind =
          ...
          failwith
            [%string
              "Trying to initialize cell %{i#Int} of memory of dtype \
               %{Layout.sexp_of_t (Ir_dtype.layout dtype)#Sexp} with a value \
  ...
  in
  { dtype; m = Act_ir.Ir.Mem.create cell_bitwidth creation_code_pos init kind }
