...
let%expect_test "colatz - chp - export" =
  ...
  [%expect
    ...
    defproc proc_0(chan!(int<32>) C0; chan?(int<32>) C1) {
      ...
      int<32> v1;
    chp {
    (v1 := 123456); (v0 := 1); ( *[ (v0 := 0); (C0?v1); ([bool(int(int((v1) != 1) = 0)) ->  [true]  [] bool(int((v1) != 1)) ->  *[ ( [true] ); (v0 := (1 + (v0))); ([bool(int(0 = ((v1) % 2))) -> v1 := ((v1) / 2) [] bool(int(int(0 = ((v1) % 2)) = 0)) -> ( [true] ); (v1 := (1 + (3 * (v1))))]) <- bool(int((v1) != 1)) ] ]); (C1!((v0))) <- bool(1) ] )
    }
    }
